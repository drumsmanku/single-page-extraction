  ...
  const renderLevels = data => {
    return data.map((item, index) => {
      ...
      if (item.children) {
        ...
        return <ExternalLink key={index} href='/' rel="noopener noreferrer" target="_blank">
        ...
        return <Box key={index}>
            <NavItemButton key={item.name} className="navItem" active={activeRoute(item.path)} >
              {item?.icon ? <item.icon sx={iconStyle(activeRoute(item.path))} /> : <BulletIcon  />}
